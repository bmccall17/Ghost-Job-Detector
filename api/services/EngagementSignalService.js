...
export class EngagementSignalService {
    ...
    async analyzeApplicationOutcomes(jobData) {
        try {
            ...
            // Calculate response times (only for responses)
            const responseTimes = outcomes
                .filter(o => o.respondedAt && o.appliedAt)
                .map(o => (new Date(o.respondedAt) - new Date(o.appliedAt)) / (1000 * 60 * 60 * 24)); // Days
    ...
    async analyzeResponsePatterns(company) {
        try {
            ...
            const responseTimes = allApplications
                .filter(app => app.respondedAt && app.appliedAt)
                .map(app => (new Date(app.respondedAt) - new Date(app.appliedAt)) / (1000 * 60 * 60 * 24));
            ...
            for (const job of similarJobs) {
                ...
                if (job.source?.lastSeenAt) {
                    ...
                    if (lastApplication) {
                        ...
                        endDate = new Date(new Date(lastApplication.appliedAt).getTime() + 30 * 24 * 60 * 60 * 1000);
                    }
                }
