...
export class SecurityValidator {
  constructor() {
    ...
    this.rateLimits = {
      ...
      parsing: { max: parseInt(process.env.AUTO_PARSING_RATE_LIMIT_PER_HOUR || '50'), window: 3600000 }, // Configurable parsing rate limit
  ...
  /**
   * Validate parsing preview request
   */
  validateParsingRequest(body) {
    const errors = [];
    ...
    // Sanitize URL only (since we're just parsing, not saving user input)
    const sanitized = {
      url: this.validateUrl(body.url)
    };
